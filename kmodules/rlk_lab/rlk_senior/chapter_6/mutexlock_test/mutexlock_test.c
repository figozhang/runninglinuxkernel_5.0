...
static void lockdep_timefunc(struct timer_list *unused);
static DEFINE_TIMER(lockdep_timer, lockdep_timefunc);

static void lockdep_timefunc(struct timer_list *unused)
{
	schedule_delayed_work(&delay_task, 10);
	mod_timer(&lockdep_timer, jiffies + msecs_to_jiffies(50));
}

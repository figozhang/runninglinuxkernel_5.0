...
rootfs_arg="root=/dev/vda rootfstype=ext4 rw console=ttyAMA0,115200"
...
show_menu() {
    echo "========================================"
    echo "    ARM64 Kernel Development Tools"
    echo "========================================"
    echo "1) menuconfig     - Configure kernel"
    echo "2) build_kernel   - Build kernel image"
    echo "3) build_rootfs   - Build rootfs image (need root)"
    echo "4) update_rootfs  - Update kernel modules (need root)"
    echo "5) run            - Run QEMU system"
    echo "6) run debug      - Run QEMU with GDB debug"
    echo "7) run_test       - Run QEMU test mode"
    echo "8) exit           - Exit"
    echo "========================================"
...
if [ $# -eq 0 ]; then
    while true; do
        ...
        case $choice in
            1)
                echo "Selected: menuconfig"
                $0 menuconfig
                ;;
            2)
                echo "Selected: build_kernel"
                $0 build_kernel
                ;;
            3)
                echo "Selected: build_rootfs"
                $0 build_rootfs
                ;;
            4)
                echo "Selected: update_rootfs"
                $0 update_rootfs
                ;;
            5)
                echo "Selected: run"
                ...
                ;;
            6)
                echo "Selected: run debug"
                $0 run debug
                ;;
            7)
                echo "Selected: run_test"
                $0 run_test
                ;;
            8)
                echo "Goodbye!"
...
run_qemu_debian_test(){
        cmd="$QEMU -m 1024 -cpu cortex-a710 -M virt,gic-version=3,its=on,iommu=smmuv3\
            -nographic $SMP -kernel arch/arm64/boot/Image \
            -append \"$kernel_arg $debug_arg $rootfs_arg $crash_arg $dyn_arg\"\
            -drive if=none,file=$rootfs_image,format=raw,id=hd0\
            -device virtio-blk-device,drive=hd0\
...
#     -m 512 \
#     -accel tcg \
#     -kernel /home/lkwu/runninglinux/runninglinuxkernel_5.0/arch/arm64/boot/Image \
#     -drive file=/home/lkwu/runninglinux/runninglinuxkernel_5.0/rootfs_debian_arm64.ext4,format=raw,if=virtio,cache=writeback \
